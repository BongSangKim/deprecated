...
class ReplayBuffer():            #3:35. et=(st,at,rt,st+1)인 튜플 e를 buffer에 저장
    def __init__(self):
        ...
        self.batch_size = 1000      #replay buffer에서 sampling할때 필요
        self.size_limit = 10000000   #buffer의 최대 크기, DQN 논문에서는 백만
...
class Qnet(nn.Module):    #Q network, torch.nn 모듈을 상속받음
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(10, 256).cuda() #nn.Linear(len(state:BSnum+UEnum*2),64)  
...
def main():
    ...
    totalE_epi_20 = []
    totalE_epi_100 = []
    epi_num_20 = np.arange(20,10001,20)
    epi_num_100 = np.arange(100,10001,100)
    BS_on_count = np.zeros(env.BSnum,dtype = float)
    BS_num_count = range(env.BSnum)
    BS_prob = np.zeros(env.BSnum, dtype = float)

    for n_epi in range(10000):  #에피소드를 100000으로 일단,
        epsilon = max(0, 0.9999-0.01*(n_epi)/75) #(0.01*(n_epi)/60)엡실론을 Linear annealing from 8% to 1%,
        ...
        for t in range(1):
            ...
            s_prime, r, done, info, info2 = env.step(a)       #a를 environment에 넣어서 다음 state(s_prime), 다음 reward, done 얻음
            ...
            if done:
                ...
                totalD_epi.append(total_DR / avg_t)
                totalE_epi.append(total_E / avg_t)
        ...
        if memory.size()>1000:       #memory가 충분히 쌓이면 train함수 호출(충분히 쌓이고 시작해야함) 
        ...
        if n_epi%20==0 and n_epi!=0:  #20에피소드마다 최근 에피소드 평균 timestep, target network를 update
            ...
            print("# of episode :{}, buffer size : {}, epsilon : {:.1f}%".format(n_epi, memory.size(), epsilon*100))
    ...
    iternum = 10000
    totalR_epi = np.array(totalR_epi)
    totalR_epi_20 = np.mean(totalR_epi.reshape(iternum // 20 , 20), axis = 1)
    totalR_epi_100 = np.mean(totalR_epi.reshape(iternum // 100, 100), axis = 1)
    totalD_epi = np.array(totalD_epi)
    totalD_epi_20 = np.mean(totalD_epi.reshape(iternum // 20 , 20), axis = 1)
    totalD_epi_100 = np.mean(totalD_epi.reshape(iternum // 100, 100), axis = 1)
    totalE_epi = np.array(totalE_epi)
    totalE_epi_20 = np.mean(totalE_epi.reshape(iternum // 20 , 20), axis = 1)
    totalE_epi_100 = np.mean(totalE_epi.reshape(iternum // 100, 100), axis = 1)
    ...
    plt.figure(2)
    plt.plot(epi_num_20, totalD_epi_20)
    plt.plot(epi_num_100, totalD_epi_100)
    plt.xlabel('episode')
    plt.ylabel('average total Datarate')
    plt.title('Result')
    plt.legend(['average 20 episode','average 100 episode'])
    
    plt.figure(3)
    plt.plot(epi_num_20, totalE_epi_20)
    plt.plot(epi_num_100, totalE_epi_100)
    plt.xlabel('episode')
    plt.ylabel('average total Energyconsumption')
    plt.title('Result')
    plt.legend(['average 20 episode','average 100 episode'])


    plt.figure(4)
    plt.bar(BS_num_count,BS_prob/1000,width = 0.7, color = "blue")
