...
class Qnet(nn.Module):    #Q network, torch.nn 모듈을 상속받음
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(65, 100).cuda() #nn.Linear(len(state:BSnum+UEnum*2),64)  
        self.fc2 = nn.Linear(100, 100).cuda() 
        self.fc3 = nn.Linear(100, 256).cuda()  #output은 action 경우의 수, 우리는 2*(BSnum)개 output되야함...
...
def main():
    ...
    for n_epi in range(50000):  #에피소드를 100000으로 일단,
        epsilon = max(0.0, 0.99-0.01*(n_epi)/300) #(0.01*(n_epi)/60)엡실론을 Linear annealing from 8% to 1%,
