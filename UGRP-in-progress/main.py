...
class ReplayBuffer():            #3:35. et=(st,at,rt,st+1)인 튜플 e를 buffer에 저장
    def __init__(self):
        ...
        self.size_limit = 1048576   #buffer의 최대 크기, DQN 논문에서는 백만
...
class Qnet(nn.Module):    #Q network, torch.nn 모듈을 상속받음
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(33, 256).cuda() #nn.Linear(len(state:BSnum+UEnum*2),64)  
        self.fc2 = nn.Linear(256, 256).cuda() 
        self.fc3 = nn.Linear(256, 64).cuda()  #output은 action 경우의 수, 우리는 2*(BSnum)개 output되야함...
...
def main():
    ...
    for n_epi in range(50000):  #에피소드를 100000으로 일단,
        epsilon = max(0.0, 0.99-0.01*(n_epi)/250) #(0.01*(n_epi)/60)엡실론을 Linear annealing from 8% to 1%,
        ...
        if memory.size()>10000:       #memory가 충분히 쌓이면 train함수 호출(충분히 쌓이고 시작해야함) 
